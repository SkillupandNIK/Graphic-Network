...
class GraphInterface
{
    ...
    private :
        ...
        grman::WidgetBox m_boite_de_boutons;

        ///Save
        grman::WidgetButton m_boutonSave;
        grman::WidgetText m_boutonSave_label;

        ///Reset
        grman::WidgetButton m_boutonReset;
        grman::WidgetText m_boutonReset_label;

        ///Delete animal
        grman::WidgetButton m_boutonDeleteAnimal;
        grman::WidgetText m_boutonDeleteAnimal_label;

        ///Connexité
        grman::WidgetText m_compoConnexe_text;
        grman::WidgetText m_k_connexite_text;
...
class Graph
{
    private :
        ...
        std::shared_ptr<GraphInterface> m_interface2 = nullptr;
    ...
    public:
        ...
        int update(std::string nom_fichier);
        void update();

        int acces_valeur_sommet(int vidx);
        int acces_coeff(int vidx);
        int acces_K(int vidx);

        void evolution_pop();


        ///Sauvegarder la position des boites et arretes
        void Save(std::string nom_fichier);

        /// Reset
        int Reset(std::string nom_fichier);
