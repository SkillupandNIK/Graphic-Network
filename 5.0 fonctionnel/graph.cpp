...
void Graph::update()
{
    ...
    *x = -1;
    ...
    //if (m_value != m_interface->m_slider_value.get_value() && mouse_unclick&1)
    {
        evolution_pop();
    }
...
int Graph::acces_coeff(int vidx)
{
    ...
    for (unsigned int i = 0; i < sommet.m_in.size(); i++)
    {
        ...
        coeff += (entrante.m_weight * m_vertices.at(entrante.m_from).m_value);
    }
...
int Graph::acces_K(int vidx)
{
    ...
    int K(m_vertices.at(vidx).m_value);
    Vertex &sommet = m_vertices.at(vidx);

    for (unsigned int i = 0; i < sommet.m_out.size(); i++)
    {
        Edge &sortante = m_edges.at(sommet.m_out[i]);
            K += (sortante.m_weight * m_vertices.at(sortante.m_to).m_value);
    }
    if (K = m_vertices.at(vidx).m_value)
        K = m_vertices.at(vidx).m_value/2;
